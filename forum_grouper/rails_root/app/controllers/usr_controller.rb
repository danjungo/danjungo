class UsrController < ApplicationController
  ...
  before_filter :authenticate_usr, :only => [ :welcome, :change_password, :edit, :delete ]
    ...
    begin
      Usr.transaction do
        ...
        if @usr.save
          ...
          flash['notice'] = 'Signup successful! Go check your e-mail and spam folder.'
          redirect_to :controller => "sttc/signup_success"
        end
      end
    rescue Exception => ex
      report_exception ex
  ...
  def change_password
    ...
    redirect_to :controller => "sttc/password_changed"
